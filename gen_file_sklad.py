...
def gen_sklad():
    ...
    desired_columns = ['OZ', 'YM', 'MM', 'WB Barcode', 'WB Артикул', 'Статус', 'Модель', 'Наличие', 'Цены OZ, YM, WB, MM']
    ...
    sklad_filtered = sklad_filtered[sklad_filtered['Цены OZ, YM, WB, MM'].str.strip().astype(bool)]
    # # Удалить пробелы и символы валюты, преобразовать к int
    sklad_filtered['Цены OZ, YM, WB, MM'] = sklad_filtered['Цены OZ, YM, WB, MM'].replace({'\s+|₽': ''}, regex=True).astype(int)
    sklad_filtered = sklad_filtered.rename(columns={'Цены OZ, YM, WB, MM': 'Цена'})
