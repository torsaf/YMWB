...
def gen_sklad():
    ...
    desired_columns = ['YM', 'WB Barcode', 'WB Артикул', 'Статус', 'Модель', 'Наличие', 'Цены YM и WB']
    ...
    # Удаление строк с пустыми значениями в 'Цены YM и WB'
    sklad_filtered = sklad_filtered[sklad_filtered['Цены YM и WB'].str.strip().astype(bool)]
    # # Удалить пробелы и символы валюты, преобразовать к int
    sklad_filtered['Цены YM и WB'] = sklad_filtered['Цены YM и WB'].replace({'\s+|₽': ''}, regex=True).astype(int)
    sklad_filtered = sklad_filtered.rename(columns={'Цены YM и WB': 'Цена'})
